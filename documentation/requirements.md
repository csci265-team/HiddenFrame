...
HiddenFrame's steganographic features require that we develop a method for inserting the payload into the carrier and successfully retrieving the payload back from the carrier. As such, we will require the ability to produce 'keys' that will mathematically describe the pixels modified. In order to accomplish this, we will utilize the algebraic concept of group generators under a binary operation mod N.
...
$$n=qx + r for some q,r \in mathbb{Z} such that 0 \leq r < x$$.
