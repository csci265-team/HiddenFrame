...
- User inivitations and signup process correctly makes users, and tracks who invited them
...
- Successful UAC denial of unauthorized users to private section, privelleged information (payloads), and privelleged features (encoding/decoding).
...
- Confirm that the HiddenFrame works in a client-server configuration (ie: works on over a LAN), and that the communication is secured (HTTPS).
...
All of the various required tests will run, at a minimum, in a Windows and in a Macintosh, environment. Futher testing utilizing Linux, Android, or other operating systems will be considered, and attempted with time permitting. Each of the test suites will be designed based off of the requirements.md, and will be generated by team members who did not directly create the driver code for the module under test. The various test cases and challenges will be desribed in the sections pertaining to their overarching test plan. The testing schedule can be see in section $BLAH$. 
