...
// default consturctor
image::image() : width(0), height(0), channels(0), filetype(), original_image(nullptr), modified_image(nullptr), coprimes(0) {};

// regular constructor - image is already in the file system
image::image(string filepath) : width(0), height(0), channels(0), filetype(), original_image(nullptr), modified_image(nullptr), coprimes(0)
{
    load_image(filepath);
};
// regular constructor - data passed by API server
image::image(const unsigned char *Data, long long unsigned int length, string ext) : width(0), height(0), channels(0), filetype(ext), original_image(nullptr), modified_image(nullptr), coprimes(0)
{
...
void image::coprime_numbers(int n)
{
    for (int i = 2; i < n; ++i)
    {
        ...
        {
            if (coprimes.size() < MAX_SIZE)
            {
                coprimes.push_back(i); // Store coprime number in array
            }
            else
            {
...
string image::generateKey(int imageSize, int channels, int idealSkipSize)
{
    ...
    for (int i = 0; i < 4; i += 2)
    {
        long long randomA = coprimes[rand() % coprimes.size()];
        while (randomA == idealSkipSize)
        {
            randomA = coprimes[rand() % coprimes.size()];
        }
